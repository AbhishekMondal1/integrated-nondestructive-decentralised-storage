const INDStorage = artifacts.require('./INDStorage.sol')

require('chai')
  .use(require('chai-as-promised'))
  .should()

contract('INDStorage', ([deployer, uploader]) => {
  let indstorage

  before(async () => {
    indstorage = await INDStorage.deployed()
  })

  describe('deployment', async () => {
    it('deploys successfully', async () => {
      const address = await indstorage.address
    ...
    it('has a name', async () => {
      const name = await indstorage.name()
      assert.equal(name, 'INDStorage')
    })
  })
  ...
  describe('file', async () => {
    ...
    before(async () => {
      result = await indstorage.uploadFile(fileHash, fileSize, fileType, fileName, fileDescription, { from: uploader })
      fileCount = await indstorage.fileCount()
    })
    ...
    it('upload file', async () => {
      ...
      await indstorage.uploadFile('', fileSize, fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have size
      await indstorage.uploadFile(fileHash, '', fileType, fileName, fileDescription, { from: uploader }).should.be.rejected;
      
      // FAILURE: File must have type
      await indstorage.uploadFile(fileHash, fileSize, '', fileName, fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have name
      await indstorage.uploadFile(fileHash, fileSize, fileType, '', fileDescription, { from: uploader }).should.be.rejected;

      // FAILURE: File must have description
      await indstorage.uploadFile(fileHash, fileSize, fileType, fileName, '', { from: uploader }).should.be.rejected;
    })

    //check from Struct
    it('lists file', async () => {
      const file = await indstorage.files(fileCount)
